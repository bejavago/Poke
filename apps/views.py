...
from apps.models import User, Poke
...
def register(request):
	...
	if not errors:
		...
		print (user.email)
		print (user.password)
		return render(request, 'index.html', {'successes': successes})
	else:
		del request.session
...
def login(request):
	...
	if len(user)<1:
		...
		return render(request, 'index.html', {'log_errors': log_errors})
	else:
		print ("Success")
...
def dashboard(request):
	...
	if "user_id" in request.session:
		...
		pokes = Poke.objects.all().values('poked_id').distinct()
		user_poke_count = Poke.objects.all().filter(poked=request.session['user_id'])
		list_of_users = Poke.objects.filter(poked=request.session['user_id']).exclude(id=request.session['user_id']).values('poker').distinct()
